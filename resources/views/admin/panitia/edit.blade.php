        ...
        <div class="admin-card admin-card-body">
            <form action="{{ route('admin.panitia.update', $panitia->id) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="nama" value="{{ old('nama', $panitia->nama) }}" class="admin-input" required>
                    @error('nama')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-900 mb-2">Username</label>
                    <input type="text" name="username" value="{{ old('username', $panitia->username) }}" class="admin-input" required>
                    @error('username')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-900 mb-2">Email</label>
                    <input type="email" name="email" value="{{ old('email', $panitia->email) }}" class="admin-input" required>
                    @error('email')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-900 mb-2">Jabatan</label>
                    <input type="text" name="jabatan" value="{{ old('jabatan', $panitia->jabatan) }}" class="admin-input" required>
