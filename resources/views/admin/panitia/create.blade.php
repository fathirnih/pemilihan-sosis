...
@section('title', 'Tambah Panitia - Admin')

@section('admin.content')
<div class="admin-page">
    <div class="admin-form-container">
        <div class="mb-6">
            <h1 class="admin-title">Tambah Panitia</h1>
            <p class="admin-subtitle">Buat akun panitia baru.</p>
        </div>
        ...
        <div class="admin-card admin-card-body">
            <form action="{{ route('admin.panitia.store') }}" method="POST" class="space-y-6">
                @csrf
                <div>
                    <label class="block text-sm font-semibold text-slate-900 mb-2">Nama</label>
                    <input type="text" name="nama" value="{{ old('nama') }}" class="admin-input" required>
                    @error('nama')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-900 mb-2">Username</label>
                    <input type="text" name="username" value="{{ old('username') }}" class="admin-input" required>
                ...
                <div>
                    ...
                    <input type="text" name="jabatan" value="{{ old('jabatan') }}" class="admin-input" required>
                ...
                <div class="admin-form-actions">
                    ...
                    <a href="{{ route('admin.panitia.index') }}" class="flex-1 admin-btn admin-btn-soft justify-center">Batal</a>
                </div>
            </form>
        </div>
    </div>
